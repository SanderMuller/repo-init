...
|---|---|---|
| `^8.4` or `^8.5` | `symplify/phpstan-rules: ^14.12` | Canonical — and the only option for a `pest` repo |
...
- **Bootstrap**: the PHPUnit stubs (`laravel-package-spatie`, `phpstan-extension`, `rector-extension`) ship `symplify/phpstan-extensions: ^12.0` — installable on every accepted floor, so a consumer that only does placeholder substitution still emits a working `composer.json` at the default `php=8.3`. The Pest stubs (`php-package`, `composer-plugin`, `laravel-package`, `filament-plugin`, `nova-tool`) ship `symplify/phpstan-rules: ^14.12` instead, because their floor is `^8.4`. A Pest stub scaffolded with `--test-framework=phpunit --php=8.3` has to swap it back to `symplify/phpstan-extensions: ^12.0`. On a PHPUnit stub, when the user picks `php=8.4` or `8.5`, replace `symplify/phpstan-extensions` with `symplify/phpstan-rules: ^14.12` in the same composer.json pass. Either way, align the `run-tests.yml` matrix with the chosen floor: drop cells below it (a `8.3` cell fails `composer update` against a higher `require.php` — and against `symplify/phpstan-rules`'s PHP ^8.4) and add cells the stub matrix lacks (the PHPUnit stubs ship `8.3`/`8.4` cells and the Pest stubs ship `8.4` cells only, so `php=8.5` needs an `8.5` cell either way).
...
|---|---|---|
| `^8.4` or `^8.5` (every `pest` repo) | `tomasvotruba/type-coverage: ^2.3` — and NO `rector/type-perfect` | 2.3 bundles the type-perfect rules; `^2.3` (not `^2.2`) is required, or resolution could land on 2.2.2, which registers no `type_perfect` params and fails boot the other way |
...
- **Bootstrap**: the PHPUnit stubs ship the PHP 8.3-floor pair (`rector/type-perfect: ^2.1` + `tomasvotruba/type-coverage: >=2.2.0 <2.2.2`) — installable on every accepted floor, so a consumer that only does placeholder substitution emits a working `composer.json` at the default `php=8.3`. When the user picks `php=8.4` or `8.5`, in the same composer.json pass **remove** `rector/type-perfect` and change `tomasvotruba/type-coverage` to `^2.3`. The Pest stubs already ship that PHP ≥ 8.4 pair — their floor is `^8.4`. A Pest stub scaffolded with `--test-framework=phpunit --php=8.3` has to add `rector/type-perfect: ^2.1` back and cap `tomasvotruba/type-coverage` at `>=2.2.0 <2.2.2`.
