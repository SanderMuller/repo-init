...
repo-init's `composer.json` requires `sandermuller/boost-core: ^0.18.0` (floor raised for the canonical `.config/boost.php` layout). The `--scope=user --all` flag combination is a 0.6.0 feature (the `--all` flag arrived alongside the plugin removal). Pre-0.6.0 boost-core supported `--scope=user` but not `--all` and did the auto-sync via the plugin instead.
