...
- [ ] `sandermuller/boost-core` — in `require` (runtime, NOT `require-dev`), canonical floor `^1.6`. A skill-bundle's consumers need boost-core present to discover and sync the shipped skills; `require-dev` is not transitive. Flag a floor below `^1.6` as NON-CANONICAL (LOW severity, additive): 1.6.0 makes `boost sync` delete only boost-owned files (the ownership manifest that fixes `laravel/boost` coexistence), adds the `boost doctor` coexistence report, and bundles the `boost-command-surfaces` skill. No breaking change — see this category's upgrade phase.
...
A skill-bundle ships no PHP source — it carries no test runner (no `pestphp/pest` / `phpunit/phpunit`) and the shared dev-dep list (PHPStan / Rector packs) does not apply. Only the three above are mandatory — `laravel/pint`, `sandermuller/boost-skills`, `stolt/lean-package-validator`.
...
- [ ] PHP floor below `^8.4` in `require.php` — repo-init floors packages at `^8.4` and `laravel-project` at `^8.5`. See `$REPO_INIT_HOME/references/version-defaults.md` "PHP".
