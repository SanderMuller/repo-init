...
- [ ] **`minimum-stability` / `prefer-stable` deviation** (LOW severity): canonical `composer.json` declares `"minimum-stability": "stable"` and `"prefer-stable": true` (the Laravel skeleton ships both; see `references/version-defaults.md`). Flag NON-CANONICAL if either key is absent, if `prefer-stable` is not `true`, or if `minimum-stability` is looser than `stable` (`dev` / `alpha` / `beta` / `RC`) without a deliberate reason. Fix: add/normalise both keys (don't loosen a passing `stable` baseline).
