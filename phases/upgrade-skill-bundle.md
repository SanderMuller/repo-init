...
2. **Ensure boost-core ≥ 0.18** (the `.config/boost/` manifest layout): bump `sandermuller/boost-core` in `require` to `^0.18.0` (skill-bundle depends on boost-core directly) and run `composer update sandermuller/boost-core`.
