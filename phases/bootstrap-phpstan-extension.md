...
**Test framework is forced to `phpunit`** for phpstan-extension (canonical for phpstan rule testing — PHPStan's `RuleTestCase` is PHPUnit-based). User can override if they really want Pest, with a warning. Pest 5 needs PHP `^8.4`, which the floor already is, so an override changes only the test deps.
...
- `composer.json` — substitute placeholders. Note: `type: phpstan-extension`, `extra.phpstan.includes: ["extension.neon"]`, `phpstan/phpstan: ^2` in `require` (NOT `require-dev` — per `references/shared-dev-deps.md` §5.1.1 exclusion), `classmap` in `autoload-dev` for `tests/Rules/stubs/`. The stub ships the canonical PHPStan dep set — `symplify/phpstan-rules: ^14.12` and `tomasvotruba/type-coverage: ^2.3`, with NO `rector/type-perfect` (keeping type-perfect alongside type-coverage >= 2.3 double-registers `MethodNodeAnalyser` and PHPStan aborts at boot). That set is the same on every accepted floor; see `references/shared-dev-deps.md` "Symplify formatter dep" and "Type-perfect dep". The stub is PHPUnit-flavoured. With `test-framework=pest` the test deps swap: add `pestphp/pest: ^5.0` + `pest-plugin-arch` + `pest-plugin-rector` + `pest-plugin-phpstan` + `pest-plugin-agent` (all `^5.0`), remove `phpunit/phpunit`, add `pestphp/pest-plugin: true` to `config.allow-plugins`, copy `tests/Pest.php`, and set `orchestra/testbench: ^11.0`. If `php=8.5`, add an `8.5` cell to `run-tests.yml`.
