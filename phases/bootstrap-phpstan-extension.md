...
- `composer.json` — substitute placeholders. Note: `type: phpstan-extension`, `extra.phpstan.includes: ["extension.neon"]`, `phpstan/phpstan: ^2` in `require` (NOT `require-dev` — per `references/shared-dev-deps.md` §5.1.1 exclusion), `classmap` in `autoload-dev` for `tests/Rules/stubs/`.
