...
**Test framework is forced to `phpunit`** for phpstan-extension (canonical for phpstan rule testing — PHPStan's `RuleTestCase` is PHPUnit-based). User can override if they really want Pest, with a warning. An override to Pest also raises the PHP floor to `^8.4` and takes the PHP >= 8.4 dep set — Pest 5 cannot install on `^8.3`.
