...
The semver-protected surface of `__VENDOR__/__PACKAGE__`. Anything documented here is covered by semver; anything outside is internal and may change without notice (including in patch releases).
...
- `__NAMESPACE__\YourPublicClass` — purpose
-->
...
Anything under `__NAMESPACE__\Internal`, anything marked `@internal` in PHPDoc, anything in `src/Support/` and `src/Concerns/`.
